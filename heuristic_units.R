...
# TODO: Remove everything related to the slack_budjet (n_r), including the loop on j
...
#   
#   debug_local <- isTRUE(debug_glb)
#   eps <- 1e-9  # Changed from 1e-12 for better floating point tolerance
#   
#   n_t <- length(treatment_scores)
#     if (n_t == 0) {
#       return(list(error = "No treated units provided"))
...
#   }
#   
#   # Check 0 at index 1 - No relevant anymore
#   # if (abs(p_sorted[1] - 0) > eps) stop("ERROR: p_sorted must start at 0")
#   
#   # -------------------------------------------------------------------------
#   # 4D DP STATE INITIALIZATION
#   # -------------------------------------------------------------------------
#   
#   dp <- array(list(), dim = c(n_t + 1, n_p, k_bound + 1))
#   backtrack <- array(list(), dim = c(n_t + 1, n_p, k_bound + 1))
...
#     cat(sprintf("DP Progress: covering k=%d values\n", k))
#     
#     # TODO: Remove this loop later!!! We don't use the slack budjet at all 
#     # for (j in 0:n_r) {
#       for (i in 0:n_t) {
#         for (v_idx in 1:n_p) {
#           
#           prev_state <- dp[[i + 1, v_idx, k + 1]]
#           
#           # IF THIS IS THE VERY START (i=0, v=1, k=0, j=0), cost is 0
#           if (i == 0 && v_idx == 1 && k == 0) {
#             prev_cost_value <- 0
#           } else if (is.null(prev_state) || is.infinite(prev_state$cost)) {
#             next # Skip unreached or broken states
...
#           if (abs(v_curr - right_border) < eps) next
#           
#           # ═══ DEBUG: Show which states are actually being processed ═══
#           # if (debug_local && i >= 296) {
#           #   cat(sprintf("  [STATE] k=%d, i=%d, v_idx=%d (PS=%.6f), cost=%.6f\n",
#           #               k, i, v_idx, v_curr, prev_cost_value))
#           # }
#           
#           for (v_next_idx in (v_idx + 1):n_p) {
#             
...
#             
#             # ═══════════════════════════════════════════════════════════
#             # Count and identify units using get_units_in_interval
#             # ═══════════════════════════════════════════════════════════
#             
#             # control_ids_in_interval <- get_controls_in_interval_dp(
#             #   control_scores, v_curr, v_next, eps
#             # )
#             
#             # Get control IDs in interval (use the utility function)
#             control_ids_in_interval <- get_units_in_interval(
#               scores = control_scores,
#               v_left = v_curr,
#               v_right = v_next,
...
#             units_in_bin <- which(T_sorted >= v_curr - eps & T_sorted <= v_next + eps)
#             
#             # We only care about units we haven't covered yet
#             new_units <- units_in_bin[units_in_bin > i]
#             n_to_capture <- length(new_units)
#             
#             
...
#               )
#               
#               n_treated_to_cover <- length(new_units)  # Use index-based count
#               next_i_candidate <- i + n_treated_to_cover
#               
#               # Safety check
#               if (next_i_candidate > n_t) {
#                 cat(sprintf("  ⚠️ next_i_candidate=%d > n_t=%d! Skipping.\n",
#                             next_i_candidate, n_t))
#                 next
#               }
#               
#               if (controls_in_interval >= n_treated_to_cover) {
...
#                 controls_ps_in_interval <- control_scores[control_ids_in_interval]
#                 
#                 # dist_power = 1 is L1 distance, dist_power = 2 is L_2^2
#                 individual_costs <- calculate_step1_cost(controls_ps_in_interval, units_to_cover_ps, dist_power)
#                 
#                 sorted_costs <- sort(individual_costs, decreasing = TRUE)
#                 
#                 # max_k_slack <- min(n_r - j, n_treated_to_cover)
#                 
#                 # for (k_slack in 0:max_k_slack) {
#                 #   current_block_cost <- if (k_slack >= n_treated_to_cover) {
#                 #     0
#                 #   } else {
#                 #     sum(sorted_costs[(k_slack + 1):n_treated_to_cover]^2)
#                 #   }
#                 #   
#                 #   new_cost <- prev_cost_value + current_block_cost
#                 #   k_new <- k + 1
#                 #   j_new <- j + k_slack
#                 #   
#                 #   # Define the target indices once to keep it clean
#                 #   ni_idx <- next_i_candidate + 1
#                 #   nv_idx <- v_next_idx
#                 #   nk_idx <- k_new + 1
#                 #   nj_idx <- j_new + 1
#                 #   
#                 #   # 2. Check the existing backtrack to see who "owns" this state currently
#                 #   # 1. Check existing state
#                 #   target_state <- dp[[ni_idx, nv_idx, nk_idx, nj_idx]]
#                 #   existing_record <- backtrack[[ni_idx, nv_idx, nk_idx, nj_idx]]
#                 #   existing_prev_i <- if(is.null(existing_record)) -1 else existing_record$prev_i_idx
#                   
#                   if (k_new <= k_bound) {
#                     
#                     # Check cost and progress
#                     # Note: target_state$cost is used if it exists, otherwise it's Inf
#                     current_stored_cost <- if(is.null(target_state)) Inf else target_state$cost
#                     
#                     is_better_cost   <- new_cost < current_stored_cost - eps
#                     is_equal_cost    <- abs(new_cost - current_stored_cost) < eps
#                     is_more_progress <- i > existing_prev_i  # Does this path come from a later unit?
#                     
#                     if (is.infinite(current_stored_cost) || is_better_cost || (is_equal_cost && is_more_progress)) {
#                       
#                       # PRESERVE: High-level progress only
#                       # if (next_i_candidate %% 10 == 0 || next_i_candidate == n_t) {
#                       #   cat(sprintf("🎯 Progress: Matched up to Unit %d/%d (Cost: %.6f, K: %d)\n", 
#                       #               next_i_candidate, n_t, new_cost, k_new))
#                       # }
#                       
#                       # --- PREPARE DATA ---
#                       captured_ids <- treatment_ids[(i + 1):next_i_candidate]
#                       
#                       # --- UPDATE DP TABLE ---
#                       dp[[ni_idx, nv_idx, nk_idx, nj_idx]] <- list(
#                         cost = new_cost,
#                         k_val = k_new
#                       )
#                       
#                       # --- UPDATE BACKTRACK TABLE ---
#                       backtrack[[ni_idx, nv_idx, nk_idx, nj_idx]] <- list(
#                         prev_i_idx = i,       # Where we came from
#                         prev_v_idx = v_idx,
#                         prev_k_idx = k,
#                         # prev_j_idx = j,
#                         interval_start = v_curr,
...
#                 nv_idx <- v_next_idx
#                 nk_idx <- k_new + 1
#                 nj_idx <- j + 1  # In Case B, j usually stays same or resets
#                 
#                 is_exit      <- (abs(v_next - right_border) < eps)
#                 is_last_ps   <- (v_next_idx == n_p)
#                 first_t_ps   <- T_sorted[1]
...
#                   target_state <- dp[[ni_idx, nv_idx, nk_idx, nj_idx]]
#                   
#                   # Use STRICT < to ensure Case B never overwrites a Case A Match of equal cost
#                   current_stored_cost <- if(is.null(target_state)) Inf else target_state$cost
#                   
#                   if (is.infinite(current_stored_cost) || prev_cost_value < current_stored_cost - eps) {
#                     
#                     # PRESERVE ONLY: If Case B reaches the very end.
#                     # if (is_exit && debug_local) {
#                     #   cat(sprintf("🏁 Case B reached Exit at K=%d\n", k_new))
#                     # }
#                     
#                     dp[[ni_idx, nv_idx, nk_idx, nj_idx]] <- list(
#                       cost = prev_cost_value,
...
#                     
#                     backtrack[[ni_idx, nv_idx, nk_idx, nj_idx]] <- list(
#                       prev_i_idx = i, prev_v_idx = v_idx, prev_k_idx = k, prev_j_idx = j,
#                       interval_start = v_curr, interval_end = v_next,
#                       n_treated = 0, n_control = controls_in_interval,
#                       is_empty = TRUE, rho = rho,
#                       treatment_unit_ids = character(0),
#                       control_unit_ids = control_ids_in_interval
#                     )
#                     
#                     # if (debug_local) {
#                     #   cat(sprintf("  [Case B] i=%d: %.4f→%.4f (k:%d→%d)\n",
#                     #               i, v_curr, v_next, k, k_new))
#                     }
#                   }
#                 }
#               }
...
# } # End k
#             
#   # ADD THIS BLOCK right before the "Searching for Optimal Final State" section
#   # It will show EXACTLY where the DP stalls
#   
#   cat("\n=== POST-LOOP DIAGNOSTIC ===\n")
#   
#   # 1. Find the furthest i reached at ANY v, k, j
#   cat("Scanning for furthest i reached...\n")
#   max_i_reached <- 0
#   max_i_details <- list()
#   
#   for (i_check in 0:n_t) {
#     for (v_check in 1:n_p) {
#       for (k_check in 1:(k_bound + 1)) {
#         for (j_check in 1:(n_r + 1)) {
#           cell <- dp[[i_check + 1, v_check, k_check, j_check]]
#           if (!is.null(cell) && is.finite(cell$cost)) {
#             if (i_check > max_i_reached) {
#               max_i_reached <- i_check
#               max_i_details <- list(
#                 i = i_check,
#                 v_idx = v_check,
#                 v_ps = p_sorted[v_check],
#                 k = k_check - 1,
#                 j = j_check - 1,
#                 cost = cell$cost
#               )
...
#   }
#   
#   cat(sprintf("Furthest i reached: %d / %d\n", max_i_reached, n_t))
#   if (length(max_i_details) > 0) {
#     cat(sprintf("  At: v_idx=%d (PS=%.6f), k=%d, j=%d, cost=%.6f\n",
#                 max_i_details$v_idx, max_i_details$v_ps,
#                 max_i_details$k, max_i_details$j, max_i_details$cost))
#   }
#   
#   # 2. Show ALL reachable states at max_i_reached
#   cat(sprintf("\nAll reachable states at i=%d:\n", max_i_reached))
#   for (v_check in 1:n_p) {
#     for (k_check in 1:(k_bound + 1)) {
#       for (j_check in 1:(n_r + 1)) {
#         cell <- dp[[max_i_reached + 1, v_check, k_check, j_check]]
#         if (!is.null(cell) && is.finite(cell$cost)) {
#           cat(sprintf("  [v_idx=%d, PS=%.6f, k=%d, j=%d] cost=%.6f\n",
#                       v_check, p_sorted[v_check],
#                       k_check - 1, j_check - 1, cell$cost))
#         }
#       }
#     }
#   }
#   
#   # 3. For EACH reachable state at max_i, check why it can't advance
#   cat(sprintf("\nWhy can't i=%d advance to i=%d?\n",
#               max_i_reached, max_i_reached + 1))
#   cat(sprintf("Next treatment unit PS: %.6f\n",
#               if (max_i_reached < n_t) T_sorted[max_i_reached + 1] else NA))
#   
#   # Find all v_idx values where i=max_i_reached is reachable
#   reachable_v <- c()
#   for (v_check in 1:n_p) {
#     for (k_check in 1:(k_bound + 1)) {
#       for (j_check in 1:(n_r + 1)) {
#         cell <- dp[[max_i_reached + 1, v_check, k_check, j_check]]
#         if (!is.null(cell) && is.finite(cell$cost)) {
#           reachable_v <- unique(c(reachable_v, v_check))
#         }
#       }
#     }
#   }
#   
#   # For each reachable v, check if Case A can fire
#   for (v_check in reachable_v) {
#     v_curr_check <- p_sorted[v_check]
#     cat(sprintf("\n  From v_idx=%d (PS=%.6f):\n", v_check, v_curr_check))
#     
#     if (max_i_reached < n_t) {
#       next_unit_ps <- T_sorted[max_i_reached + 1]
#       
#       # Check all v_next values that could capture the next unit
#       valid_v_next <- which(
#         p_sorted > next_unit_ps - eps & 
#           p_sorted <= v_curr_check + delta_dp + eps &
#           p_sorted > v_curr_check
#       )
#       
#       if (length(valid_v_next) == 0) {
#         cat(sprintf("    ✗ No v_next in range (%.6f, %.6f] that reaches next unit (%.6f)\n",
#                     v_curr_check, v_curr_check + delta_dp, next_unit_ps))
#         cat(sprintf("    Gap to next unit: %.6f (delta=%.6f)\n",
#                     next_unit_ps - v_curr_check, delta_dp))
#       } else {
#         cat(sprintf("    Possible v_next values for Case A:\n"))
#         for (vn in valid_v_next[1:min(5, length(valid_v_next))]) {
#           vn_ps <- p_sorted[vn]
#           width <- vn_ps - v_curr_check
#           
#           # Count treatments
#           t_in_bin <- which(T_sorted >= v_curr_check & T_sorted <= vn_ps + eps)
#           t_new <- t_in_bin[t_in_bin > max_i_reached]
#           
#           # Count controls  
#           c_in_bin <- sum(control_scores > v_curr_check + eps & 
#                             control_scores <= vn_ps + eps)
#           
#           cat(sprintf("      v_next=%.6f: width=%.4f, T=%d, C=%d → Case A? %s\n",
#                       vn_ps, width,
#                       length(t_new), c_in_bin,
#                       (length(t_new) > 0 && t_new[1] == max_i_reached + 1 &&
#                          width <= delta_dp + eps && c_in_bin >= length(t_new))))
#         }
#       }
#       
#       # Check Case B: can we jump to the next unit?
#       dist_to_next <- next_unit_ps - v_curr_check
#       rho_needed <- ceiling((dist_to_next - eps) / delta_dp)
#       cat(sprintf("    Case B to next unit (%.6f): dist=%.6f, rho=%d\n",
#                   next_unit_ps, dist_to_next, rho_needed))
#     }
#   }
#   
#   # 4. Quick check: is the issue that i=0 never advances at all?
#   cat("\n\nStates reachable at i=1 (first treatment captured):\n")
#   count_i1 <- 0
#   for (v_check in 1:n_p) {
#     for (k_check in 1:(k_bound + 1)) {
#       for (j_check in 1:(n_r + 1)) {
#         cell <- dp[[2, v_check, k_check, j_check]]  # i=1 → index 2
#         if (!is.null(cell) && is.finite(cell$cost)) {
#           count_i1 <- count_i1 + 1
#           if (count_i1 <= 5) {
#             cat(sprintf("  [v_idx=%d, PS=%.6f, k=%d] cost=%.6f\n",
#                         v_check, p_sorted[v_check], k_check - 1, cell$cost))
#           }
#         }
#       }
#     }
#   }
#   cat(sprintf("Total states at i=1: %d\n", count_i1))
#   if (count_i1 == 0) {
#     cat("✗ i=1 NEVER REACHED - Case A never fires from the very start!\n")
#     cat("  This means the jump to PS=0.574 worked, but Case A from there fails.\n")
#     cat("\n  Manual check at (i=0, v_idx=2, k=12):\n")
#     
#     v_curr_check <- p_sorted[2]  # 0.573936
#     cat(sprintf("  v_curr = %.6f\n", v_curr_check))
#     cat(sprintf("  T_sorted[1] = %.6f (first treatment)\n", T_sorted[1]))
#     
#     # Check first few v_next values
#     for (vn_idx in 3:min(10, n_p)) {
#       vn_ps <- p_sorted[vn_idx]
#       width <- vn_ps - v_curr_check
#       
#       t_in_bin <- which(T_sorted >= v_curr_check & T_sorted <= vn_ps + eps)
#       t_new <- t_in_bin[t_in_bin > 0]  # i=0, so > 0
#       c_count <- sum(control_scores > v_curr_check + eps & control_scores <= vn_ps + eps)
#       
#       cat(sprintf("  v_next[%d]=%.6f: width=%.6f, T_in_bin=%d, T_new=%d, C=%d\n",
#                   vn_idx, vn_ps, width, length(t_in_bin), length(t_new), c_count))
#       
#       if (length(t_new) > 0) {
#         cat(sprintf("    first_new=%d, targeting=1, match=%s\n",
#                     t_new[1], t_new[1] == 1))
#       }
#       
#       if (width > delta_dp) break
#     }
#   }
#   
#   # ═══════════════════════════════════════════════════════════════════
...
#   for (k_val in 0:k_bound) {
#     for (j_val in 0:n_r) {
#       state <- dp[[n_t + 1, v_term_idx, k_val + 1, j_val + 1]]
#       
#       if (!is.null(state) && state$cost < best_final_cost - 1e-9) {
#         best_final_state <- list(
#           final_cost = state$cost,
...
#   }
#   
#   # --- CORRECTED 4D DEBUG CHECK ---
#   n_t <- length(treatment_scores)
#   n_p <- length(p_sorted)
#   
#   cat("\n--- Checking 4D DP Terminal Reachability ---\n")
#   
#   # We check the "Terminal Slice": Last Treatment Unit (n_t+1) and Last PS (n_p)
#   # across all possible K and J values.
#   terminal_slice_costs <- c()
#   
#   for (k_idx in 1:(k_bound + 1)) {
#     for (j_idx in 1:(n_r + 1)) {
#       cell <- dp[[n_t + 1, n_p, k_idx, j_idx]]
#       if (!is.null(cell) && is.finite(cell$cost)) {
#         terminal_slice_costs <- c(terminal_slice_costs, cell$cost)
#       }
#     }
...
#     for (i_idx in 1:(n_t + 1)) {
#       reached_this_i <- FALSE
#       # Check if any state in this 'i' level is finite
#       for (v_idx in 1:n_p) {
#         for (k_idx in 1:(k_bound + 1)) {
#           for (j_idx in 1:(n_r + 1)) {
#             if (is.finite(dp[[i_idx, v_idx, k_idx, j_idx]]$cost)) {
#               reached_this_i <- TRUE; break
#             }
#           }
#           if(reached_this_i) break
...
#   }
#   
#   cat(sprintf("✅ Success! Optimal Final Cost: %.6f at State [k_idx=%d, j_idx=%d]\n", 
#               best_final_state$final_cost, 
#               best_final_state$final_k_idx, 
#               best_final_state$final_j_idx))
#   
#   # ═══════════════════════════════════════════════════════════════════
#   # EXTRACT BACKTRACK AND RECONSTRUCT
#   # ═══════════════════════════════════════════════════════════════════
...
#   best_v_idx <- best_final_state$final_v_idx
#   best_k_idx <- best_final_state$final_k_idx
#   best_j_idx <- best_final_state$final_j_idx
#   
#   best_k <- best_k_idx - 1  # Convert index to value
#   best_j <- best_j_idx - 1  # Convert index to value
#   
#   cat(sprintf("\n=== Extracting Final Backtrack Record ===\n"))
#   cat(sprintf("State: [i=%d, v_idx=%d, k=%d, j=%d]\n",
#               n_t, best_v_idx, best_k, best_j))
#   
#   final_backtrack_record <- backtrack[[best_i_idx, best_v_idx, best_k_idx, best_j_idx]]
#   
#   if (is.null(final_backtrack_record)) {
#     cat("ERROR: Final backtrack record is NULL.\n")
#     return(list(error = "Backtrack path incomplete"))
...
#               final_backtrack_record$prev_i_idx,
#               final_backtrack_record$prev_v_idx,
#               p_sorted[final_backtrack_record$prev_v_idx],
#               final_backtrack_record$prev_k_idx,
#               final_backtrack_record$prev_j_idx))
#   
#   # Check if path reaches right_border
#   if (p_sorted[best_v_idx] < right_border - 1e-6) {
#     cat(sprintf("\n⚠️ WARNING: Optimal path ends at PS=%.6f (not right_border)\n",
#                 p_sorted[best_v_idx]))
#     
#     remaining_distance <- right_border - p_sorted[best_v_idx]
...
#     intervals_dist_optimal = reconstruction_result$intervals,
#     redundant_unit_ids = character(0),  # TODO: When using redundant units
#     n_t_covered = n_t
#   ))
# }

# Modified version for 3D instead of 4D
...
#     
#     # --- 2. DATA ACCESS ---
#     bp_list <- backtrack[[current_i_idx, current_v_idx, current_k_idx, current_j_idx]]
#     
#     if (is.null(bp_list)) {
#       cat(sprintf("  Stopping: Found NULL at [i=%d, v=%d, k=%d, j=%d]\n", 
#                   current_i_idx, current_v_idx, current_k_idx, current_j_idx))
#       break
#     }
#     
#     # Use your specific sentinel name (i_idx or prev_i) 
#     # Check if this is the origin state
#     if (is.null(bp_list$prev_i_idx) || bp_list$prev_i_idx == -1) {
#       cat(sprintf("  Reached Origin sentinel at Step %d. Done.\n", step))
#       break
...
#     # Extract treatment IDs
#     treatment_ids <- bp_list$treatment_unit_ids
#     # Ensure it's a character vector for sprintf
#     if (is.null(treatment_ids)) {
#       treatment_ids <- character(0)
#     } else {
#       treatment_ids <- as.character(unlist(treatment_ids))
...
#     
#     # --- 4. PRINTING ---
#     # Safe extraction of numeric values for printing
#     s_ps <- as.numeric(bp_list$interval_start)
#     e_ps <- as.numeric(bp_list$interval_end)
#     k_c  <- as.integer(if(length(treatment_ids) == 0) bp_list$rho else 1)
#     label <- if(length(treatment_ids) == 0) "EMPTY" else "NON-EMPTY"
...
#                 step, s_ps, e_ps, label, k_c))
#     
#     # if (length(treatment_ids) > 0) {
#     #   cat(sprintf("  Treatment IDs: %s\n", paste(treatment_ids, collapse=", ")))
#     # }
#     # if (length(control_ids) > 0) {
#     #   cat(sprintf("  Control IDs: %s\n", paste(head(control_ids, 10), collapse=", ")))
#     # }
#     
#     # Extract n_control from the bp_list (Image 3 confirms this exists)
#     n_c_stored <- as.integer(bp_list$n_control %||% 0)
#     
#     # ← NEW: Verify control count matches
#     if (length(control_ids) != n_c_stored) {
#       cat(sprintf("  ⚠️ Warning: Control count mismatch - stored: %d, IDs: %d\n",
#                   n_c_stored, length(control_ids)))
#     }
#     
#     # ═══ THE MISSING STEP: SAVE THE DATA ═══
#     new_interval <- list(
#       start_ps  = s_ps,
#       end_ps    = e_ps,
#       is_empty  = (length(treatment_ids) == 0),
...
#       n_treated = length(treatment_ids),
#       k_cost    = k_c,
#       treatment_ids = treatment_ids,  # ← Changed from "ids"
#       control_ids = control_ids        # ← NEW
#     )
#     
...
#     
#     # --- 5. MOVE TO PREVIOUS STATE ---
#     # IMPORTANT: If your DP stored 'prev_i' as a 0-based logical value, 
#     # you MUST add +1 here. If it stored it as an index, leave it as is.
#     # Based on our previous discussion, adding +1 is usually safer if you used 0-based logic.
#     
#     next_i_logical <- bp_list$prev_i_idx
#     next_v_idx <- bp_list$prev_v_idx
#     next_k_logical <- bp_list$prev_k_idx
#     next_j_logical <- bp_list$prev_j_idx
#     
#     if (next_i_logical == -1) {
#       cat(sprintf("  Success: Reached the origin (Sentinel -1) at Step %d.\n", step))
#       break
#     }
#     
#     # 3. CONVERT logical values to R indices for the NEXT loop iteration
#     # This is the "Update" step
#     current_i_idx <- next_i_logical + 1
#     current_v_idx <- next_v_idx
#     current_k_idx <- next_k_logical + 1
#     current_j_idx <- next_j_logical + 1
#   }
#   
#   # Reverse the list to get chronological order (PS 0 -> 1)
#   final_intervals_list <- rev(intervals_list)
#   
#   # 2. Check if it's empty to avoid crashes
#   if (length(final_intervals_list) == 0) {
#     return(list(intervals = data.frame(), redundant_ids = NULL))
#   }
#   
#   # 3. Use a more robust conversion method
#   # This converts each list element to a 1-row data frame, then stacks them
#   intervals_df <- do.call(rbind, lapply(final_intervals_list, function(x) {
#     data.frame(
#       start_ps  = as.numeric(x$start_ps),
#       end_ps    = as.numeric(x$end_ps),
...
#   }))
#   
#   # 4. Attach the ID lists as attributes 
#   # (Since a data frame column can't easily hold lists of varying lengths)
#   attr(intervals_df, "treatment_unit_ids") <- lapply(final_intervals_list, `[[`, "treatment_ids")
#   attr(intervals_df, "control_unit_ids") <- lapply(final_intervals_list, `[[`, "control_ids")  # ← NEW
#   
#   # ← NEW: Verification
#   cat("\n=== Verification ===\n")
#   total_treatments <- sum(sapply(attr(intervals_df, "treatment_unit_ids"), length))
...
#   }
#   
#   return(list(intervals = intervals_df)) # Add Step 4 logic back as needed
# }
...
calculate_ls_cost <- function(
  ...
  cost_result <- calculate_distance_from_bounds_units(
    ...
    X_all = X_all,                   
    S_inv = S_inv,           # Passed
    cov_cols = cov_cols,     # ADD THIS - It was missing!
    delta_dp = delta_dp,     # ADD THIS - It was missing!
    K_max = K_max,
    p_sorted = p_sorted,
    # n_r = n_r,
    intervals = intervals,
...
# standardize_ties <- function(bounds_idx, treatment_ids, treatment_scores, 
#                              intervals_df, cov_df, X_all, S_inv, 
#                              data_config, id_var) {
#   
#   TIE_EPS <- 1e-12
#   NUDGE   <- 1e-10
...
#   
#   # Get the control lists from attributes once
#   c_id_list <- attr(intervals_df, "control_unit_ids")
#   
#   # --- CRITICAL SAFETY CHECK ---
#   # If we have 0 or 1 bins, there are no interior boundaries to shift across.
#   # if (is.null(c_id_list) || length(c_id_list) < 2) {
#   #   return(list(changed = FALSE, ids = curr_ids, scores = curr_scores))
#   # }
#     
#   # If bounds_idx has 5 elements, this now goes 1, 2, 3. 
#   # b=3 will compare c_id_list[[3]] (Left) and c_id_list[[4]] (Right).
#   
#   
#   # --- ROBUST LOOP HEADER ---
#   # Use seq_len to avoid the 1:0 downward counting trap
#   # length(c_id_list) - 1 is the number of internal boundaries
#   # for (b in seq_len(length(c_id_list) - 1)) {
#   for (b in 1:(length(c_id_list) - 1)) {
#     # boundary_ps <- curr_scores[bounds_idx[b]]
#     
#     boundary_ps <- curr_scores[bounds_idx[b + 1]]
#     tie_indices <- which(abs(curr_scores - boundary_ps) < TIE_EPS)
#     
#     if (length(tie_indices) == 0) next
#     
#     # Left bin is b, Right bin is b+1
#     left_ctrls <- c_id_list[[b]]
#     right_ctrls <- c_id_list[[b + 1]]
#     
#     # Apply simplified Right-Shift Logic
#     for (idx in tie_indices) {
#       d_left <- if(length(left_ctrls) > 0) 
#         calc_mahalanobis_to_group(curr_ids[idx], left_ctrls, cov_df, X_all, S_inv, id_var) else Inf
#       
#       d_right <- if(length(right_ctrls) > 0) 
#         calc_mahalanobis_to_group(curr_ids[idx], right_ctrls, cov_df, X_all, S_inv, id_var) else Inf
#       
#       # Only nudge right if it's strictly better
#       if (d_right < d_left && is.finite(d_right)) {
#         curr_scores[idx] <- boundary_ps + NUDGE
#         any_changed <- TRUE
#       }
#     }
#     
#     # Re-sort after each boundary to keep indices valid for next boundary
#     if (any_changed) {
#       reorder <- order(curr_scores)
#       curr_ids <- curr_ids[reorder]
#       curr_scores <- curr_scores[reorder]
#     }
#   }
#   
#   return(list(changed = any_changed, ids = curr_ids, scores = curr_scores))
# }
...
local_search_units <- function(
  ...
  while (improved && iter <= max_iter) {
    ...
    for (b in 2:(best_bounds_len - 1)) {
      ...
      # # LAYER 2: Tie-Breaking (only if boundary didn't move)
      # if (enable_tie_breaking && !boundary_shifted) {
      #   
      #   
      #   #debug
      #   # browser()
      #   
      #   # Standardize ties for the current configuration
      #   tie_res <- standardize_ties(
      #     best_bounds_idx, curr_ids, curr_scores, 
      #     best_intervals, cov_df, X_all, S_inv, data_config, id_var
      #   )
      #   
      #   if (tie_res$changed) {
      #     cand <- call_ls_cost(best_bounds_idx)
      #     
      #     # Improved previous best cost - Needs update
      #     if (is.finite(cand$cost) && cand$cost < best_cost - COST_EPS) {
      #       best_cost <- cand$cost
      #       best_intervals <- cand$intervals
      #       best_k <- cand$k_actual
      #       curr_ids <- tie_res$ids
      #       curr_scores <- tie_res$scores
      #       improved <- TRUE
      #       cat(sprintf("  ✓ Iter %d: Ties standardized at boundary %d, cost %.6f\n", iter, b, best_cost))
      #       
      #       # Log the "improvement event"
      #       trace_len <- length(trace) + 1
      #       trace[[trace_len]] <- list(
...
enhanced_local_search_units <- function(
  ...
  while (improved && iter <= max_iter) {
    ...
    # ═══════════════════════════════════════════════════════════════════
    # LAYER 2: Unified Tie-Breaking, only if no improvement achieved so far
    # ═══════════════════════════════════════════════════════════════════
    # if (enable_tie_breaking && !improved) {
    #   
    #   #debug
    #   # browser()
    #   
    #   # Use the standardized unified function
    #   tie_fix <- standardize_ties(
    #     best_bounds_idx, curr_ids, curr_scores, 
    #     best_intervals, cov_df, X_all, S_inv, data_config, id_var
    #   )
    #   
    #   if (tie_fix$changed) {
    #     res <- call_ls_cost(best_bounds_idx)
    #     if (is.finite(res$cost) && res$cost < best_cost - COST_EPS) {
    #       best_cost <- res$cost
    #       best_intervals <- res$intervals
    #       best_k <- res$k_actual
    #       curr_ids <- tie_fix$ids
    #       curr_scores <- tie_fix$scores
    #       improved <- TRUE
    #       cat(sprintf("  ✓ Iter %d: Standardized ties, cost %.6f\n", iter, best_cost))
    #       
    #       
    #       # Log the "improvement event"
    #       trace_len <- length(trace) + 1
  ...
  for (iter in 1:max_iter) {
    ...
    if (is.finite(neighbor_cost)) {
      ...
      if (delta_E < 0 || runif(1) < exp(-delta_E / temp)) {
        ...
        # 4. TIE-BREAKING & BEST UPDATING
        # Only run expensive refinement if we are close to or better than best
       # if (current_cost < (best_cost + temp_buffer)) { # Slightly wider window for SA
          
          #debug
          # browser()
          
      #     if (enable_tie_breaking) {
      #       
      #       #debug
      #       # browser() 
      #       
      #       # Ensure we are not at the very first boundary (b=0)
      #       tie_fix <- standardize_ties(
      #         current_bounds, curr_ids, curr_scores, 
      #         current_intervals, cov_df, X_all, S_inv, data_config, id_var
      #       )
      #       if (tie_fix$changed) {
      #         refined_res <- call_ls_cost(current_bounds)
      #         if (is.finite(refined_res$cost)) {
      #           current_cost <- refined_res$cost
      #           current_intervals <- refined_res$intervals
      #           curr_ids <- tie_fix$ids
      #           curr_scores <- tie_fix$scores
      #         }
      #       }
